"""Seedance image-to-video API client."""
...
class SeedanceI2VClient(LoggedHTTPClientMixin, I2VModelClient):
    """Thin wrapper around a Seedance-compatible image-to-video API."""

    name = "seedance"

    def __init__(self, api_key: str | None = None, base_url: str | None = None):
        self.api_key = api_key or require_env("SEEDANCE_API_KEY")
        self.base_url = (base_url or os.getenv("SEEDANCE_BASE_URL") or "https://api.seedance.com").rstrip("/")
    ...
    def download(self, job_id: str, output_path: str) -> str:
        ...
        if not url:
            raise ValueError(f"Seedance job {job_id} has no downloadable video URL")
